...
void rotateX(float angle, std::vector<Vector3>& point);
void rotateY(float angle, std::vector<Vector3>& point);
void rotateZ(float angle, std::vector<Vector3>& point);
...
void rotateX(float angle, std::vector<Vector3>& point)
{
    ...
    {
        float newY = point[i].y * cosA - point[i].z * sinA;
        float newZ = point[i].z * cosA + point[i].y * sinA;
        point[i].y = newY;
        point[i].z = newZ;
    }
}
...
void rotateY(float angle, std::vector<Vector3>& point)
{
    ...
    for(int i = 0; i < point.size(); ++i)
    {
        float newX = point[i].x * cosA - point[i].z * sinA;
        float newZ = point[i].z * cosA + point[i].x * sinA;
        point[i].x = newX;
        point[i].z = newZ;
    }
}
...
void rotateZ(float angle, std::vector<Vector3>& point)
{
    ...
    for(int i = 0; i < point.size(); ++i)
    {
        float newX = point[i].x * cosA - point[i].y * sinA;
        float newY = point[i].y * cosA + point[i].x * sinA;
        point[i].x = newX;
        point[i].y = newY;
    }
}