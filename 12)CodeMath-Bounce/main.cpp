...
int main()
{
    ...
    {
        particles[i].SetAll(center.getX(), center.getY(), getRand1f() * 10, getRand1f() * 2 * PI, 1, 1, 10, -0.9);
    }
...
void RemoveDeadParticles(std::vector<MyParticle>& particles)
{
    for(int i = particles.size() - 1; i >= 0; --i)
    {
        if(particles[i].GetPosition().getX() > 1280 + particles[i].GetRadius() ||
            particles[i].GetPosition().getX() < 0 - particles[i].GetRadius() ||
            particles[i].GetPosition().getY() > 720 + particles[i].GetRadius() ||
            particles[i].GetPosition().getY() < 0 - particles[i].GetRadius())
            {
